...
def convert_date_format(date_str: str, source: Source) -> str:
    match source:
        case Source.SOLANA:
            dt = datetime.strptime(date_str, "%Y-%m-%d %H:%M:%S")   # 2024-04-08 08:49:04
            dt_utc = dt.replace(tzinfo=pytz.UTC)
            oslo_tz = pytz.timezone('Europe/Oslo')
            dt_oslo = dt_utc.astimezone(oslo_tz)
            return dt_oslo.strftime("%Y-%m-%d %H:%M:%S %z")
        case _:
            return ""
...
with open('default.csv', 'r') as csvfile_in:
    ...
    output_filename = "til_kryptosekken {}.csv".format(datetime.now().strftime("%Y-%m-%d %H.%M.%S"))
            ...
            if tx_type == SolanaTxType.UNKNOWN.value:
                ...
                if received_amount != "" and received_currency != "":
                    ...
                    if burnt_nft_id != None:
                        ...
                        comment = "Orca pool withdraw"

                    csv_writer.writerow([date_str_out, tx_type, received_amount, received_currency, 
                                         "",    # Ut
                                         "",    # Ut-Valuta
                                         fee, fee_currency, exchange, make_notat(comment, txid)])
                    continue
